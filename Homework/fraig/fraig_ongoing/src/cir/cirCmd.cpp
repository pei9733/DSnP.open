...
initCirCmd()
{
   if (!(cmdMgr->regCmd("CIRRead", 4, new CirReadCmd) &&
         ...
         cmdMgr->regCmd("CIRCec", 4, new CirCecCmd)
      )) {
...
//----------------------------------------------------------------------
//    CIRCec <-File (string compare.aag)>
//----------------------------------------------------------------------
CmdExecStatus
CirCecCmd::exec(const string& option)
{
    if (!cirMgr) {
      cerr << "Error: circuit is not yet constructed!!" << endl;
      return CMD_EXEC_ERROR;
   ...
   CirMgr* cm1 = 0;
   cirMgr->equivalaceChecking(cm1);
   return CMD_EXEC_DONE;
}

void
CirCecCmd::usage(ostream& os) const
{
   os << "Usage: CIRCec <-File (string compare.aag)>" << endl;
}

void
CirCecCmd::help() const
{
   cout << setw(15) << left << "CIRCec: "
        << "Check if the current circuit is equivalent to the one in compare.aag" << endl;
}